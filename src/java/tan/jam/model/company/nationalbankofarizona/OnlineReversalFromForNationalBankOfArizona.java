 ...
 * @author Lenovo
 */
public class OnlineReversalFromForNationalBankOfArizona {
    ...
    public OnlineReversalFromForNationalBankOfArizona(List<String> lines) {
        this.lines = lines;
        startExecution();
    }
   ...
   private List<Double> captureAmountFromSelectedRecords() {
        ...
        for (String s : this.filteredBlock) {
            String result = FileUtils.getResultFromPattern("Reverse|RETURN|REVERSAL|Return", s);
            if (!result.startsWith("NoMatch")) {
                String[] temp = s.split("\\s+");
                main.add(FileUtils.convertStringToDouble(FileUtils.getDollarSignRemovedValue(temp[1])));
            }
        }
        return main;
    }
