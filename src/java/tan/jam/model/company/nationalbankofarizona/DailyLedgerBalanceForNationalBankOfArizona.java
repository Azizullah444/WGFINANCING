 ...
 * @author Lenovo
 */
public class DailyLedgerBalanceForNationalBankOfArizona {
    ...
    DailyLedgerBalanceForNationalBankOfArizona(List<String> lines) {
        this.lines = lines;
        startExecution();
    }
...
private List<Double> captureSelectedBlock() {
        ...
        int endIndex = FileUtils.findIndexBySingleValue(lines, "This\\s+page\\s+intentionally\\s+left\\s+blank|^\\s*Processed\\s+\\d{1,2}/\\d{1,2}/\\{2,4}\\s+$\\d+");
