 ...
 * @author Lenovo
 */
public class OnlineTransferToForGuilfordSavingBank {
    ...
    public OnlineTransferToForGuilfordSavingBank(List<String> initialBlock) {
    ...
    private void captureDateDescriptionAmount() {
        ...
        for (String s : list) {
            ...
            String res = FileUtils.getResultFromPattern("CHK\\s+\\d{8,}", desc);
    ...
    private List<String> captureSelectedRecordsBlock() {
        ...
        for (String s : list) { 
            String result = FileUtils.getResultFromPattern("INTERNET\\s+TFR\\s+TO", s);
