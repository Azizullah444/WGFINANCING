 ...
 * @author Lenovo
 */
public class DailyLedgerBalanceForGuilfordSavingBank {
    ...
    DailyLedgerBalanceForGuilfordSavingBank(List<String> lines) {
        this.lines = lines;
        startExecution();
    }
...
private List<Double> captureSelectedBlock() {
        String start = "Summary of Activity by Date";
        String end = "\\d{2}/\\d{2}\\s+Ending\\s+Balance";
        List<Double> mainList = new ArrayList<>();
        List<String> temp = new ArrayList<>();
        List<String> list = FileUtils.getSelectedBlock(lines, start, end);
        for (String s : list) { 
            String result = FileUtils.getResultFromPattern("^\\d{2}/\\d{2}", s); //System.out.println("Line "+s);
        ...
        for (String s : temp) { 
            ...
            String value="0";
                 if (result[result.length-1].endsWith("D")) 
                 {
