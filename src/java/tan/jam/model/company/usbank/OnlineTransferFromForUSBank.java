 ...
 * @author Lenovo
 */
public class OnlineTransferFromForUSBank {
    ...
    public OnlineTransferFromForUSBank(List<String> lines) {
        this.lines = lines;
        startExecution();
    }
    ...
    private List<Double> captureAmountFromSelectedRecords() {
        ...
        for (String s : this.filteredBlock) {
            String result = FileUtils.getResultFromPattern("Online\\s+Transfer\\s+From|Mobile\\s+Banking\\s+Transfer|Internet\\s+Banking\\s+Transfer\\s+From|Online\\s+Transfer\\s+.+From", s);
