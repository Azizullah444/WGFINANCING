 ...
 * @author Lenovo
 */
public class KeywordsForUSBank {
    ...
    public KeywordsForUSBank(List<String> lines, List<String> words) {
        this.words = words;
        this.lines = lines;
    ...
    private double extractAmountFromKeywordListAndReturnSum() {
     ...
     private void captureSelectedBlockForDebit() {
        ...
        for (int a = 0; a < List.size(); a++) {
            String value = FileUtils.getNegativeSignDetectedValue(FileUtils.getLastElementFromArray(List.get(a)));
            value = FileUtils.getDollarSignRemovedValue(value);
            double d = FileUtils.convertStringToDouble(value);
            d= Math.abs(d);
