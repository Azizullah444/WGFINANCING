 ...
 * @author Lenovo
 */
public class CredentialsForNevadaStateBank {
    ...
    public CredentialsForNevadaStateBank(String[] pages) {
        this.pages = pages;
    ...
    public CredentialsForNevadaStateBank(String companyName) {
    ...
    private double captureTotalDeposits() {
       ...
       for(int a=0;a<lines.size();a++){   
           if(lines.get(a).matches("^\\s*Previous\\s+Balance.+")){
    ...
    private String captureAccountNumber() {
        ...
        if (result.startsWith("NoMatch")){ result = FileUtils.getResultFromPattern("\\s+\\d{8,}", pages[0]); }
