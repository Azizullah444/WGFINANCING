 ...
 * @author Lenovo
 */
public class DailyLedgerBalanceForCommunityBank {
    ...
    DailyLedgerBalanceForCommunityBank(List<String> lines) {
        this.lines = lines;
        startExecution();
    }
    ...
    private double findAverageValues() {
       ...
       for(String s: lines){
           if(s.contains("Average Ledger")){
               ...
               average=FileUtils.convertStringToDouble(FileUtils.getNegativeSignDetectedValue(temp[temp.length-1]));
               break;
           }
       }
        return average;
...
private List<Double> captureSelectedBlock() {
        String start = "^\\s*DAILY\\s+BALANCE\\s+INFORMATION|^\\s*Daily\\s+Balance\\s+Information";
        String end = "^\\s*IN\\s+CASE\\s+OF\\s+ERRORS\\s+OR\\s+QUESTIONS\\s+ABOUT\\s+ELECTRONIC\\s+TRANSFERS,\\s+CONTACT\\s+YOUR\\s+LOCAL";
