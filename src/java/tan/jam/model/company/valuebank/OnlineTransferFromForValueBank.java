 ...
 * @author Lenovo
 */
public class OnlineTransferFromForValueBank {
    ...
    public OnlineTransferFromForValueBank(List<String> lines) {
        this.lines = lines;
        startExecution();
    }
    ...
    private List<Double> captureAmountFromSelectedRecords() {
        ...
        for (String s : this.filteredBlock) {
            String result = FileUtils.getResultFromPattern("Internet\\s+Deposit", s);
            if (!result.startsWith("NoMatch")) { 
                String[] temp = s.split("\\s+");   if (temp.length>3){
    ...
    private List<String> captureFilteredBlock() {
        ...
        for (String s : list) {
            String result = FileUtils.getResultFromPattern("^\\d{2}/\\d{2}/\\d{2,4}", s);
            if (!result.startsWith("NoMatch")) { 
                main.add(s);
            }
        }
