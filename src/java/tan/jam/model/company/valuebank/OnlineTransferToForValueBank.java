 ...
 * @author Lenovo
 */
public class OnlineTransferToForValueBank {
    ...
    public OnlineTransferToForValueBank(List<String> initialBlock) {
    ...
    private List<String> captureSelectedRecordsBlock() {
        ...
        for (int a = 0; a < list.size(); a++) { 
            String result = FileUtils.getResultFromPattern("Internet\\s+Withdrawal", list.get(a));
            if (!result.startsWith("NoMatch") && list.get(a).matches("^\\s*\\d{1,2}/\\d{1,2}/\\d{2,4}.+")) { 
                String desc=list.get(a)+" "+list.get(a+1);
                String res = FileUtils.getResultFromPattern("DDA\\s+\\d+", desc);
