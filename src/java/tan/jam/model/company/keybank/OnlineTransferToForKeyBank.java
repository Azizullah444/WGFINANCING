 ...
 * @author Lenovo
 */
public class OnlineTransferToForKeyBank {
    ...
    public OnlineTransferToForKeyBank(List<String> initialBlock) {
    ...
    private void captureDateDescriptionAmount() {
        ...
        for (String s : list) {
            ...
            String res = FileUtils.getResultFromPattern("DDA\\s+\\d+\\s+\\d{4}", desc);
    ...
    private List<String> captureSelectedRecordsBlock() {
        ...
        for (String s : list) { 
            String result = FileUtils.getResultFromPattern("Trf\\s+To", s);
