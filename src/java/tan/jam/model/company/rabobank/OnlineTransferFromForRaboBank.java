 ...
 * @author Lenovo
 */
public class OnlineTransferFromForRaboBank {
    ...
    public OnlineTransferFromForRaboBank(List<String> lines) {
        this.lines = lines;
        startExecution();
    }
    ...
    private List<Double> captureAmountFromSelectedRecords() {
        ...
        for (String s : this.filteredBlock) {
            String result = FileUtils.getResultFromPattern("Online\\s+Banking\\s+Transfer\\s+From|Tlr\\s+Transfer|Online\\s+Banking\\s+transfer\\s+from|Online\\s+scheduled\\s+transfer\\s+from", s);
