 ...
 * @author Lenovo
 */
public class OnlineTransferToForBBT {
    ...
    public OnlineTransferToForBBT(List<String> initialBlock) {
    ...
    private void captureDateDescriptionAmount() {
        ...
        for (String s : list) {
            ...
            String res = FileUtils.getResultFromPattern("CHECKING\\s+\\d{4,}", desc);
    ...
    private List<String> captureSelectedRecordsBlock() {
        ...
        for (String s : list) {
            String result = FileUtils.getResultFromPattern("ONLINE\\s+TRANSFER\\s+TRANSFER\\s+TO|TRANSFER\\s+TRANSFER\\s+TO", s);
