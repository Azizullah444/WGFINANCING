 ...
 * @author Lenovo
 */
public class DailyLedgerBalanceForBBT {
    ...
    DailyLedgerBalanceForBBT(List<String> lines) {
        this.lines = lines;
        startExecution();
    }
    ...
    private double captureTotalPrevious() {
         ...
         double d = 0;
       for(String s: lines){
           if(s.contains("Your previous balance as of")){
               result = s;
               break;
           }
       }
...
private void CaptureChecksBlock() {
        String start = "DATE\\s+CHECK";
        String end = "Total\\s+checks";
        List<String> CheckDate = new ArrayList<>();
        List<Double> CheckAmount = new ArrayList<>();
         ...
         for (String s : temp) {
            ...
            for (int a = 0; a < result.length; a++) {
                if (result[a].matches("\\d{1,2}/\\d{1,2}")) {
                    ...
                    String value="0";               
                    if(result[a+1].contains(".")){value=FileUtils.getDollarSignRemovedValue(result[a+1]);}
                    else if( a+2<result.length && result[a+2].contains("."))  { value = FileUtils.getDollarSignRemovedValue(result[a+2]);}
...
private void CaptureDebitBlock() {
        String start = "Total\\s+checks";
        String end = "Total\\s+other\\s+withdrawals,\\s+debits\\s+and\\s+service\\s+charges";
        List<String> DebitDate = new ArrayList<>();
        List<Double> DebitAmount = new ArrayList<>();
        List<String> temp = new ArrayList<>();
        List<String> list = FileUtils.getSelectedBlock(lines, start, end);
...
private void CaptureCreditBlock() {
        String start = "Total\\s+other\\s+withdrawals,\\s+debits\\s+and\\s+service\\s+charges";
        String end = "Total\\s+deposits,\\s+credits\\s+and\\s+interest";
        List<String> CreditDate = new ArrayList<>();
        List<Double> CreditAmount = new ArrayList<>();
        List<String> temp = new ArrayList<>();
        List<String> list = FileUtils.getSelectedBlock(lines, start, end);
