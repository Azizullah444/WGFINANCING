 ...
 * @author Lenovo
 */
public class KeywordsForSantaCruzCountryBank {
    ...
    public KeywordsForSantaCruzCountryBank(List<String> lines, List<String> words) {
        this.words = words;
        this.lines = lines;
        ...
        for (int x = 0; x < debitBlock.size(); x++) {
            for (int a = 0; a < words.size(); a++) {
                ...
                if (!result.startsWith("NoMatch")) {
                    for (int R = x; R > 0; R--) {
                        if (debitBlock.get(R).matches("^\\s*\\d{1,2}/\\d{1,2}/\\d{2,4}.+")) {
    ...
    private List<String> captureSelectedBlockForDeposit() {
        ...
        for (int x = 0; x < depositBlock.size(); x++) {
            for (int a = 0; a < words.size(); a++) {
                ...
                if (!result.startsWith("NoMatch")) {
                    for (int R = x; R > 0; R--) {
                        if (depositBlock.get(R).matches("^\\s*\\d{1,2}/\\d{1,2}/\\d{2,4}.+")) {
