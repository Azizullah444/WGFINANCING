 ...
 * @author Lenovo
 */
public class OnlineTransferToForFirstBank {
    ...
    public OnlineTransferToForFirstBank(List<String> initialBlock) {
    ...
    private void captureDateDescriptionAmount() {
        ...
        for (String s : list) {
            ...
            String res = FileUtils.getResultFromPattern("\\d{6,}", desc);
    ...
    private List<String> captureSelectedRecordsBlock() {
        ...
        for (String s : list) { 
            String result = FileUtils.getResultFromPattern("Internet\\s+Transfer\\s+To", s);
            if (!result.startsWith("NoMatch") && s.matches("^\\s*\\d{2}/\\d{2}.+")) { 
                main.add(s);
            }
