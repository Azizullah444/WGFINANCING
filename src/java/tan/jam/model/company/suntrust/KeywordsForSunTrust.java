 ...
 * @author Lenovo
 */
public class KeywordsForSunTrust {
    ...
    public KeywordsForSunTrust(List<String> lines, List<String> words) {
        this.words = words;
        this.lines = lines;
    ...
    private double extractAmountFromKeywordListAndReturnSum() {
        ...
        for (int a = 0; a < this.selectedBlock.size(); a++) {
            String value = FileUtils.getSecondtElementFromArray(this.selectedBlock.get(a));
     ...
     private void captureSelectedBlockForDebit() {
        ...
        for (int a = 0; a < List.size(); a++) {
            String value = FileUtils.getSecondtElementFromArray(List.get(a));
            value = FileUtils.getDollarSignRemovedValue(value);
            double d = FileUtils.convertStringToDouble(value);
            d= Math.abs(d);
