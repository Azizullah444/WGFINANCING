 ...
 * @author Lenovo
 */
public class DailyLedgerBalanceForBestBank {
     private List<String> lines;
    private List<Double> selectedBlock;
    private int noOfNegativeValues;
    private double minValue;

    DailyLedgerBalanceForBestBank(List<String> lines) {
        this.lines = lines;
        startExecution();
    }
...
private List<Double> captureSelectedBlock() {
        ...
        String end = "^\\s*OVERDRAFT/RETURN\\s+ITEM\\s+FEES|Thank\\s+you\\s+for\\s+banking\\s+with\\s+Best\\s+Bank";
