 ...
 * @author Lenovo
 */
public class CredentialsForNBTBank{
    ...
    CredentialsForNBTBank(String[] page,List<String> lines) {
        this.pages = page;
        this.lines = lines;
        startExecution();
    }
    ...
    private void startExecution() {
     ...
     private double captureTotalDeposits() {
         ...
         double d = 0;
       for(String s: lines){
           if(s.matches("^\\s*\\+?\\s*DEPOSITS\\s*/\\s*CREDITS.+")){
               result = s;
               break;
           }
       }
     ...
     private String captureDate() {
        ...
        for (int a = 0; a < lines.size(); a++) {
            String d = FileUtils.getResultFromPattern("\\d{1,2}-\\d{1,2}-\\d{1,2}", this.lines.get(a));
