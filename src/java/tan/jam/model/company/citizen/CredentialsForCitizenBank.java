 ...
 * @author Lenovo
 */
public class CredentialsForCitizenBank {
    ...
    public CredentialsForCitizenBank(String[] pages) {
        this.pages = pages;
    ...
    private double captureTotalDeposits() {
         ...
         double d = 0;
       for(String s: lines){
           if(s.matches("Deposits\\s*&\\s*Credits.+")){
               result = s;
               break;
           }
       }
    ...
    private String captureDate() {
        String result = FileUtils.getResultFromPattern("through\\s+\\D+\\s+\\d{2},\\s+\\d{4}", pages[0]);
        String[] temp=result.split("\\s+");
        return temp[1];
    }

    private String captureAccountNumber() {
        String result = FileUtils.getResultFromPattern("\\d+-\\d+-\\d+-\\d+", pages[0]);
