 ...
 * @author Lenovo
 */
public class CredentialsForPopularCommunityBank{
    ...
    CredentialsForPopularCommunityBank(String[] page,List<String> lines) {
        this.pages = page;
        this.lines = lines;
        startExecution();
    }
    ...
    private void startExecution() {
        this.companyName = "PopularCommunity";
     ...
     private double captureTotalDeposits() {
         ...
         double d = 0;
       for(String s: lines){
           if(s.matches("^\\s*\\d{1,2}-\\d{1,2}\\s+Ending\\s+Totals.+")){
               result = s;
               break;
           }
       }
       ...
       if(result.length() > 1){
           String[] temp = result.split("\\s+"); 
            d = FileUtils.convertStringToDouble(FileUtils.getDollarSignRemovedValue(temp[temp.length-3]));
       }
       return d;
    }
     ...
     private String captureDate() {
        ...
        for (int a = 0; a < lines.size(); a++) {
            String d = FileUtils.getResultFromPattern("This\\s+statement:\\s*\\D+\\s+\\d{1,2},\\s+\\d{4}", this.lines.get(a));
