 ...
 * @author Lenovo
 */
public class OnlineTransferToForCityNationalBank {
    ...
    public OnlineTransferToForCityNationalBank(List<String> initialBlock) {
    ...
    private void captureDateDescriptionAmount() {
        ...
        for (String s : list) {
            ...
            if (!temp[temp.length-1].matches("\\d*(,\\d+)?(,\\d+)?(,\\d+)?\\.\\d+")){ continue; }
            date.add(temp[0]);
            ...
            String res = FileUtils.getResultFromPattern("LINE\\s+\\d+", desc);
    ...
    private List<String> captureSelectedRecordsBlock() {
        ...
        for (String s : list) {
            String result = FileUtils.getResultFromPattern("Automatic\\s+TRANSFER\\s+TO", s);
