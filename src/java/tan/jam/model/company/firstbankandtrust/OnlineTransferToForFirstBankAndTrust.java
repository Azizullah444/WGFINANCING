 ...
 * @author Lenovo
 */
public class OnlineTransferToForFirstBankAndTrust {
    ...
    public OnlineTransferToForFirstBankAndTrust(List<String> initialBlock) {
    ...
    private void captureDateDescriptionAmount() {
        ...
        for (String s : list) {
            ...
            date.add(temp[0]);
            amount.add(FileUtils.convertStringToDouble(FileUtils.getDollarSignRemovedValue(FileUtils.getNegativeSignDetectedValue(temp[temp.length-1]))));
            String desc = "";
    ...
    private List<String> captureSelectedRecordsBlock() {
        ...
        for (int a = 0; a < list.size(); a++) { 
            String result = FileUtils.getResultFromPattern("Transf\\s+to", list.get(a));
            if (!result.startsWith("NoMatch")) { 
                String desc=list.get(a)+" "+list.get(a+1)+" "+list.get(a+2);
                String res = FileUtils.getResultFromPattern("Confirmation\\s+number\\s+\\d+", desc);
