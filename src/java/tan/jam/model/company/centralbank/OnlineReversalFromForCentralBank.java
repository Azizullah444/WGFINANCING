 ...
 * @author Lenovo
 */
public class OnlineReversalFromForCentralBank {
    ...
    public OnlineReversalFromForCentralBank(List<String> lines) {
        this.lines = lines;
        startExecution();
    }
   ...
   private List<Double> captureAmountFromSelectedRecords() {
        ...
        for (String s : this.filteredBlock) {
            String result = FileUtils.getResultFromPattern("RTN|REFUND|RETURNED", s);
    ...
    private List<String> captureFilteredBlock() {
        ...
        for (String s : list) {
            String result = FileUtils.getResultFromPattern("^\\s*\\d{1,2}/\\d{1,2}/\\d{2}", s);
            if (!result.startsWith("NoMatch")) {
                main.add(s);
            }
        }
