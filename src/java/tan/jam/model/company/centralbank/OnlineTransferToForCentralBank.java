 ...
 * @author Lenovo
 */
public class OnlineTransferToForCentralBank {
    ...
    public OnlineTransferToForCentralBank(List<String> initialBlock) {
    ...
    private void captureDateDescriptionAmount() {
        ...
        for (String s : list) {
            ...
            String res = FileUtils.getResultFromPattern("TO\\s+\\d{8,}", desc);
    ...
    private List<String> captureSelectedRecordsBlock() {
        ...
        for (String s : list) { 
            String result = FileUtils.getResultFromPattern("IB\\s+TFR\\s+TO", s);
