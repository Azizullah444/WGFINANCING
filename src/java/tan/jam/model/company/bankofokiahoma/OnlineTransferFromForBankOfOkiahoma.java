 ...
 * @author Lenovo
 */
public class OnlineTransferFromForBankOfOkiahoma {
    ...
    public OnlineTransferFromForBankOfOkiahoma(List<String> lines) {
        this.lines = lines;
        startExecution();
    }
    ...
    private List<Double> captureAmountFromSelectedRecords() {
        ...
        for (String s : this.filteredBlock) {
            String result = FileUtils.getResultFromPattern("Funds\\s+Transfer\\s+From|XFER\\s+FROM", s);
