 ...
 * @author Lenovo
 */
public class DailyLedgerBalanceForWoodForest {
     private List<String> lines;
    private List<Double> selectedBlock;
    private int noOfNegativeValues;
    private double minValue;

    DailyLedgerBalanceForWoodForest(List<String> lines) {
        this.lines = lines;
        startExecution();
    }
    ...
    private double findMinValue() {
        double d = 0;
        for(String s: lines){
            String result = FileUtils.getResultFromPattern("Minimum\\s+Balance\\s+on\\s+\\d{1,2}-\\d{1,2}", s);
...
private List<Double> captureSelectedBlock() {
        String start = "^\\s*Daily\\s+Closing\\s+Balance\\s+Summary";
        String end = "^\\s*Account\\s+Item\\s+Images\\s+Total";
