 ...
 * @author Lenovo
 */
public class KeywordsForBMOHarrisBank {
    ...
    public KeywordsForBMOHarrisBank(List<String> lines, List<String> words) {
        this.words = words;
        this.lines = lines;
            ...
            for (int x = 0; x < debitBlock.size(); x++) {
              for (int a = 0; a < words.size(); a++) {
                ...
                if (!result.startsWith("NoMatch")) { 
                   for(int R=x;R>0;R--)
                    {    
                        if(debitBlock.get(R).matches("^\\s*\\D{3}\\s+\\d{1,2}.+"))
                        {  
              ...
              for (int a = 0; a < words.size(); a++) {
                ...
                String result = FileUtils.getResultFromPattern(wordLine, depositBlock.get(x).toLowerCase());
                  if (!result.startsWith("NoMatch")) { 
                   for(int R=x;R>0;R--)
                    {    
                        if(depositBlock.get(R).matches("^\\s*\\D{3}\\s+\\d{1,2}.+"))
                        {  
