 ...
 * @author Lenovo
 */
public class OnlineTransferFromForBMOHarrisBank {
    ...
    public OnlineTransferFromForBMOHarrisBank(List<String> lines) {
        this.lines = lines;
        startExecution();
    }
    ...
    private List<Double> captureAmountFromSelectedRecords() {
        ...
        for (String s : this.filteredBlock) {
            String result = FileUtils.getResultFromPattern("Online\\s+Transfer\\s+From|Mobile\\s+Banking\\s+Transfer|PC\\s+TRANSFER\\s+CREDIT", s);
            if (!result.startsWith("NoMatch")) {
                String[] temp = s.split("\\s+");
                main.add(FileUtils.convertStringToDouble(FileUtils.getDollarSignRemovedValue(temp[2])));
            }
        }
        return main;
    }
