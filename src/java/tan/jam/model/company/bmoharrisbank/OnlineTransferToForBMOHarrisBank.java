 ...
 * @author Lenovo
 */
public class OnlineTransferToForBMOHarrisBank {
    ...
    public OnlineTransferToForBMOHarrisBank(List<String> initialBlock) {
    ...
    private void captureDateDescriptionAmount() {
        ...
        for (String s : list) {
            ...
            date.add(temp[0]);
            amount.add(FileUtils.convertStringToDouble(FileUtils.getDollarSignRemovedValue(FileUtils.getNegativeSignDetectedValue(temp[2]))));
            String desc = "";
    ...
    private List<String> captureSelectedRecordsBlock() {
        ...
        for (String s : list) {
            String result = FileUtils.getResultFromPattern("Mobile\\s+Banking\\s+Transfer\\s+To", s);
