 ...
 * @author Lenovo
 */
public class DebitsForBBVACompass {
    ...
    public DebitsForBBVACompass(List<String> lines) {
        this.lines = lines;
        date = new ArrayList<>();
    ...
    private void captureOnlineTransferToRecords() {
        ...
        for (int a = 0; a < this.selectedBlock.size(); a++) {
            String result = FileUtils.getResultFromPattern("ONLINE\\s*BANKING\\s*TRANSFER\\s*TO|TRANSFER\\s*TO\\s*ACCOUNT", this.selectedBlock.get(a));
            ...
            if (!result.startsWith("NoMatch") && selectedBlock.get(a).matches("^\\s*\\d{1,2}/\\d{1,2}.+")) {
                ...
                date.add(temp[0]);
                String value = FileUtils.getDollarSignRemovedValue(FileUtils.getBracketRemovedValue(FileUtils.getSecondLastElementFromArray(this.selectedBlock.get(a))));
        ...
        for (int a = 0; a < indexFinder.size(); a++) {
           ...
           // String val2 = selectedBlock.get(indexFinder.get(a)+1);
            ...
            String ch = FileUtils.getResultFromPattern("ACCOUNT\\s*X+\\d+|ACCT\\s*\\*\\d+", result);
    ...
    private List<String> captureSelectedBlock() {
        int startIndex = FileUtils.findIndexBySingleValue(lines, "^\\s*Transaction\\s*History");
