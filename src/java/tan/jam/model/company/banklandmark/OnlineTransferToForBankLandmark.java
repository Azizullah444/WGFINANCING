 ...
 * @author Lenovo
 */
public class OnlineTransferToForBankLandmark {
    ...
    public OnlineTransferToForBankLandmark(List<String> initialBlock) {
    ...
    private void captureDateDescriptionAmount() {
        ...
        for (String s : list) {
            ...
            String res = FileUtils.getResultFromPattern("TO\\s+\\d+", desc);
    ...
    private List<String> captureSelectedRecordsBlock() {
        ...
        for (String s : list) { 
            String result = FileUtils.getResultFromPattern("TRANSFER\\s+TO", s);
            if (!result.startsWith("NoMatch") && s.matches("^\\s*\\d{2}/\\d{2}/\\d{2}.+")) { 
                main.add(s);
            }
