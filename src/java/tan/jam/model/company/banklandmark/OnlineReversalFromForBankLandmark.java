 ...
 * @author Lenovo
 */
public class OnlineReversalFromForBankLandmark {
    ...
    public OnlineReversalFromForBankLandmark(List<String> lines) {
        this.lines = lines;
        startExecution();
    }
   ...
   private List<Double> captureAmountFromSelectedRecords() {
    ...
    private List<String> captureFilteredBlock() {
        ...
        for (String s : list) {
            String result = FileUtils.getResultFromPattern("^\\d{2}/\\d{2}/\\d{2}", s);
            if (!result.startsWith("NoMatch")) {
                main.add(s);
            }
        }
