 ...
 * @author Lenovo
 */
public class KeywordsForMTBank{
    ...
    public KeywordsForMTBank(List<String> lines, List<String> words) {
        this.words = words;
        this.lines = lines;
            ...
            for (int x = 0; x < lines.size(); x++) {
              for (int a = 0; a < words.size(); a++) {
                ...
                if (!result.startsWith("NoMatch")) { 
                   for(int R=x;R>0;R--)
                    {    
                        if(lines.get(R).matches("^\\s*\\d{1,2}/\\d{1,2}/\\d{2,4}.+"))
                        {  
    ...
    private List<String> captureInitialBlock() {
       ...
       return  FileUtils.getSelectedBlock(lines, "^\\s*ACCOUNT\\s+ACTIVITY", "NUMBER\\s+OF\\s+DEPOSITS/CHECKS\\s+PAID|CHECKS\\s+PAID\\s+SUMMARY");
