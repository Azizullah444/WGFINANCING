 ...
 * @author Lenovo
 */
public class KeywordsForWellsRiver {
    ...
    public KeywordsForWellsRiver(List<String> lines, List<String> words) {
        this.words = words;
        this.lines = lines;
    ...
    private double extractAmountFromKeywordListAndReturnSum() {
        ...
        for (int a = 0; a < this.selectedBlock.size(); a++) {
            ...
            String value = FileUtils.getDollarSignRemovedValue(temp[1]);
            ...
            for (int x = 0; x < debitBlock.size(); x++) {
              for (int a = 0; a < words.size(); a++) {
                ...
                if (!result.startsWith("NoMatch")) { 
                    ...
                    String result1=FileUtils.getResultFromPattern("^\\d{1,2}/\\d{1,2}", debitBlock.get(x));
            ...
            for (int x = 0; x < depositBlock.size(); x++) {
              for (int a = 0; a < words.size(); a++) {
                ...
                String result = FileUtils.getResultFromPattern(wordLine, depositBlock.get(x).toLowerCase());
                  if (!result.startsWith("NoMatch")) { 
                    String result1=FileUtils.getResultFromPattern("^\\d{1,2}/\\d{1,2}", depositBlock.get(x));
