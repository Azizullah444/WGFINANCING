 ...
 * @author Lenovo
 */
public class DailyLedgerBalanceForWellsRiver {
    ...
    DailyLedgerBalanceForWellsRiver(List<String> lines) {
        this.lines = lines;
        startExecution();
    }
...
private List<Double> captureSelectedBlock() {
        int startIndex = FileUtils.findIndexBySingleValue(lines, "^\\s*Balance\\s+by\\s+Date");
        int endIndex = FileUtils.findIndexBySingleValue(lines, startIndex, "^\\s*Deposit,\\s+Amount");
