 ...
 * @author Lenovo
 */
public class CredentialsForFirstMerit{
    ...
    CredentialsForFirstMerit(String[] page,List<String> lines) {
        this.pages = page;
        this.lines = lines;
        startExecution();
    }
    ...
    private void startExecution() {
     ...
     private double captureTotalDeposits() {
         ...
         double d = 0;
       for(String s: lines){
           if(s.contains("Deposits and Credits")){
               result = s;
               break;
           }
       }
     ...
     private String captureDate() {
        ...
        for (int a = 0; a < lines.size(); a++) {
            String d = FileUtils.getResultFromPattern("\\D+\\s+1,\\s+\\d{4}", this.lines.get(a));
      ...
      private String captureAccountNumber() {
        String result = FileUtils.getTargetByMatch(pages[0], "\\d{2}+\\s+\\d{4}+\\s+\\d{4}+");
