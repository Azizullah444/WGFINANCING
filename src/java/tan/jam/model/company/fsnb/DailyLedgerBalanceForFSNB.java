 ...
 * @author Lenovo
 */
public class DailyLedgerBalanceForFSNB {
    ...
    public DailyLedgerBalanceForFSNB(List<String> lines) {
        this.lines = lines;
        startExecution();
    }
 ...
 private List<Double> captureSelectedBlock() {
        int startIndex = FileUtils.findIndexBySingleValue(lines, "ACTIVITY\\s+IN\\s+DATE\\s+ORDER");
        int endIndex = FileUtils.findIndexBySingleValue(lines, "CHECKS\\s+IN\\s+NUMBER\\s+ORDER|WE\\s+APPRECIATE\\s+YOUR\\s+BUSINESS");
        ...
        for(int a=0;a< selected.size();a++){
            ...
            if(line.length > 1){ 
                    mainList.add(FileUtils.convertStringToDouble(FileUtils.getDollarSignRemovedValue(FileUtils.getNegativeSignDetectedValue(FileUtils.getODRemovedValue(line[line.length-1])))));
