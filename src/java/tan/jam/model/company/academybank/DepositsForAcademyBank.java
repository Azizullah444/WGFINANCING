 ...
 * @author Lenovo
 */
public class DepositsForAcademyBank {
    ...
    public DepositsForAcademyBank(List<String> lines, List<String> keywords) {
        this.lines = lines;
    ...
    private List<Double> captureOnlineTransferFromBlock() {
        List<Double> dValue = new ArrayList<>();
       List<String> list = FileUtils.getResultFromBlock("Online\\s+Transfer\\s+From", selectedBlock);
    ...
    private List<String> captureSelectedBlock() {
        ...
        String start = "^\\s*Account\\s+Activity";
        String end = "^\\s*Daily\\s+Balances";
        List<String> list = FileUtils.getSelectedBlock(lines, start, end);
        for (String s : list) {
            String result = FileUtils.getResultFromPattern("^\\d{1,2}/\\d{1,2}/\\d{4}", s);
