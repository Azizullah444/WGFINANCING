 ...
 * @author Lenovo
 */
public class OnlineTransferToForFirstBank2 {
    ...
    public OnlineTransferToForFirstBank2(List<String> initialBlock) {
    ...
    private List<String> captureSelectedRecordsBlock() {
        ...
        for (int a = 0; a < list.size(); a++) { 
            String result = FileUtils.getResultFromPattern("Trsfr", list.get(a));
            if (!result.startsWith("NoMatch")) { 
                String desc=list.get(a)+" "+list.get(a+1)+" "+list.get(a+2);
                String res = FileUtils.getResultFromPattern("DEP\\s+\\d+", desc);
