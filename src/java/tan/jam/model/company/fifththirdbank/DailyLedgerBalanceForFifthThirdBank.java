 ...
 * @author Lenovo
 */
public class DailyLedgerBalanceForFifthThirdBank {
    ...
    DailyLedgerBalanceForFifthThirdBank(List<String> lines) {
        this.lines = lines;
        startExecution();
    }
...
private List<Double> captureSelectedBlock() {
        ...
        int endIndex = FileUtils.findIndexBySingleValue(lines, startIndex, "AT FIFTH THIRD, WE ARE COMMITTED TO BEING CLEAR|This page intentionally left blank.");
            ...
            for (int a = 0; a < result.length; a++) {
               String result1 = FileUtils.getResultFromPattern("\\d{2}/\\d{2}", result[a]);
                 if (!result1.startsWith("NoMatch") && a+1<result.length) {
                    String value = FileUtils.getDollarSignRemovedValue(FileUtils.getBracketRemovedValue(result[a+1]));
