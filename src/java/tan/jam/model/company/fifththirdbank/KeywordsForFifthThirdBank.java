 ...
 * @author Lenovo
 */
public class KeywordsForFifthThirdBank {
    ...
    public KeywordsForFifthThirdBank(List<String> lines, List<String> words) {
        this.words = words;
        this.lines = lines;
        ...
        List<String> list = new ArrayList<>();
            for (int x = 0; x < depositBlock.size(); x++) {
               for (int a = 0; a < words.size(); a++) {
                ...
                if (!result.startsWith("NoMatch")) { System.out.println("Passsss"); System.out.println("Line "+depositBlock.get(x));
                   for(int R=x;R>0;R--)
                    {    
