 ...
 * @author Lenovo
 */
public class DebitsForBlackHills {
    ...
    public DebitsForBlackHills(List<String> lines) {
        this.lines = lines;
        date = new ArrayList<>();
    ...
    private void captureOnlineTransferToRecords() {
        ...
        for (int a = 0; a < this.selectedBlock.size(); a++) {
            String result = FileUtils.getResultFromPattern("Online\\s+Transfer\\s+to", this.selectedBlock.get(a));
            ...
            if (!result.startsWith("NoMatch")) {
                ...
                date.add(temp[0]);
                String value = FileUtils.getDollarSignRemovedValue(FileUtils.getNegativeSignDetectedValue((FileUtils.getSecondLastElementFromArray(this.selectedBlock.get(a)))));
        ...
        for (int a = 0; a < indexFinder.size(); a++) {
            ...
            String ch = FileUtils.getResultFromPattern("XXX{1,}\\d{4}|xxx{1,}\\d{4}|\\d{4}", result);
