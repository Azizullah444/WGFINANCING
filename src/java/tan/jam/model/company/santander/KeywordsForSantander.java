 ...
 * @author Lenovo
 */
public class KeywordsForSantander {
    ...
    public KeywordsForSantander(List<String> lines, List<String> words) {
        this.words = words;
        this.lines = lines;
    ...
    private double extractAmountFromKeywordListAndReturnSum() {
        ...
        for (int a = 0; a < this.selectedBlock.size(); a++) {
            String value = FileUtils.getSecondLastElementFromArray(this.selectedBlock.get(a));
    ...
    private List<String> captureSelectedBlock() {
        ...
        for (int x = 0; x < lines.size(); x++) {
            for (int a = 0; a < words.size(); a++) {
                String wordLine = words.get(a).toLowerCase().replaceAll("\\s+", "");
                String result = FileUtils.getResultFromPattern(wordLine, lines.get(x).toLowerCase().replaceAll("\\s+", ""));
                if (!result.startsWith("NoMatch")) {
                    for (int R = x; R > 0; R--) {
                        if (lines.get(R).matches("^\\s*\\d{1,2}-\\d{1,2}.+")) {
    ...
    private List<String> captureInitialBlock() {
        ...
        int endIndex = FileUtils.findIndexBySingleValue(lines, "^\\d{1,2}-\\d{1,2}\\s*Ending\\s*Balance|^\\s*Ending\\s*Balance");
