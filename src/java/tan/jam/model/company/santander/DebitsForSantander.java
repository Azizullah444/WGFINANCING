 ...
 * @author Lenovo
 */
public class DebitsForSantander {
    ...
    public DebitsForSantander(List<String> lines) {
        this.lines = lines;
        date = new ArrayList<>();
    ...
    private void captureOnlineTransferToRecords() {
        ...
        for (int a = 0; a < this.selectedBlock.size(); a++) {
            String result = FileUtils.getResultFromPattern("INTERNET\\s*TRANSFER\\s*TO", this.selectedBlock.get(a));
        ...
        for (int a = 0; a < indexFinder.size(); a++) {
            ...
            String ch = FileUtils.getResultFromPattern("ACCT\\s*\\d+\\s*\\d+", result);
