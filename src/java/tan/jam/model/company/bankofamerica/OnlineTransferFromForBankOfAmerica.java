 ...
 * @author Lenovo
 */
public class OnlineTransferFromForBankOfAmerica {
    ...
    public OnlineTransferFromForBankOfAmerica(List<String> lines) {
        this.lines = lines;
        startExecution();
    }
    ...
    private List<Double> captureAmountFromSelectedRecords() {
        ...
        for (String s : this.filteredBlock) {
            String result = FileUtils.getResultFromPattern("Online\\s+Banking\\s+Transfer\\s+From|Tlr\\s+Transfer|Online\\s+Banking\\s+transfer\\s+from|Online\\s+scheduled\\s+transfer\\s+from|OVERDRAFT\\s+PROTECTION\\s+FROM", s);
            if (!result.startsWith("NoMatch")) {
                String[] temp = s.split(" ");
                //System.out.println("online from " + s + "     "+temp[temp.length-1]);
