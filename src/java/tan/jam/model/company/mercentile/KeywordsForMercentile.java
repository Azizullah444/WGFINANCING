 ...
 * @author Lenovo
 */
public class KeywordsForMercentile{
    ...
    public KeywordsForMercentile(List<String> lines, List<String> words) {
        this.words = words;
        this.lines = lines;
    ...
    private double extractAmountFromKeywordListAndReturnSum() {
        ...
        for (int a = 0; a < this.selectedBlock.size(); a++) {
            ...
            datelist.add(temp[temp.length-2]);
            list.add(d);
            ...
            for (int x = 0; x < debitBlock.size(); x++) {
              for (int a = 0; a < words.size(); a++) {
                ...
                if (!result.startsWith("NoMatch")) { 
                   for(int R=x;R>0;R--)
                    {    
                        if(debitBlock.get(R).matches(".+\\d{1,2}/\\d{1,2}.+"))
                        {  
            ...
            for (int x = 0; x < depositBlock.size(); x++) {
               for (int a = 0; a < words.size(); a++) {
                ...
                if (!result.startsWith("NoMatch")) { 
                   for(int R=x;R>0;R--)
                    {    
                        if(depositBlock.get(R).matches(".+\\d{1,2}/\\d{1,2}.+"))
                        {  
