 ...
 * @author Lenovo
 */
public class OnlineTransferFromForChase {
    ...
    public OnlineTransferFromForChase(List<String> lines) {
        this.lines = lines;
        startExecution();
    }
    ...
    private List<Double> captureAmountFromSelectedRecords() {
        ...
        for (String s : this.filteredBlock) {
            String result = FileUtils.getResultFromPattern("Online\\s+Transfer\\s+From|Online\\s+transfer\\s+from|Transfer\\s+From\\s+Chk", s);
