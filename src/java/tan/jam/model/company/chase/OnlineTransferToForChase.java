 ...
 * @author Lenovo
 */
public class OnlineTransferToForChase {
    ...
    public OnlineTransferToForChase(List<String> initialBlock) {
    ...
    private void captureDateDescriptionAmount() {
        ...
        for (String s : list) {
            ...
            String res = FileUtils.getResultFromPattern("Chk ...?\\d{4,}|CHK ...?\\d{4,}|Mma ...?\\d{4,}|Chk\\s+Xx+\\d+", desc);
    ...
    private List<String> captureSelectedRecordsBlock() {
        ...
        for (String s : list) {
            String result = FileUtils.getResultFromPattern("Online\\s+Transfer\\s+To|Online\\s+transfer\\s+to|Transfer\\s+To", s);
