 ...
 * @author Lenovo
 */
public class DailyLedgerBalanceForMetropolitan {
    ...
    DailyLedgerBalanceForMetropolitan(List<String> lines) {
        this.lines = lines;
        startExecution();
    }
...
private List<Double> captureSelectedBlock() {
        String start = "^\\s*\\*+\\s*BALANCE\\s+BY\\s+DATE";
        String end = "^\\s*HOW\\s+TO\\s+RECONCILE\\s+YOUR\\s+CHECK\\s+BOOK\\s+BALANCE\\s+TO\\s+THE\\s+STATEMENT\\s+BALANCE";
