 ...
 * @author Lenovo
 */
public class DailyLedgerBalanceForNewMexico {
    ...
    DailyLedgerBalanceForNewMexico(List<String> lines) {
        this.lines = lines;
        startExecution();
    }
...
private List<Double> captureSelectedBlock() {
        int startIndex = FileUtils.findIndexBySingleValue(lines,"^\\s*BALANCE\\s+SUMMARY");
        ...
        for (String s : list) { 
            String result = FileUtils.getResultFromPattern("^\\d{2}/\\d{2}/\\d{2}", s); //System.out.println("Line "+s);
        ...
        { 
            ...
            {
               String result1 = FileUtils.getResultFromPattern("\\d{2}/\\d{2}/\\d{2}", result[a]);
